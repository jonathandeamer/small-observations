...
make ingest-dry   # python -m ingest --dry-run (reports slugs; writes and moves nothing)
...
- `geocode.py` — reverse-geocode (lat, lon) → (country, city) via OpenStreetMap Nominatim. 1 req/sec rate-limited (their policy); retries once on 429. Queries at zoom 10, then retries at zoom 14 if no settlement key comes back (central Belfast returns only a historic boundary at zoom 10). `_CITY_ALIASES` maps council-district names to everyday ones (`Belfast City District` → `Belfast`). On-disk cache at `.cache/geocode.json` keyed by coords rounded to 4 decimals (~10m); a cached `city: null` is never re-queried, so delete that entry to retry.
...
- `__main__.py` — CLI: scan `_ingest/`, process each, move originals to `_ingest/_processed/` on success. `--dry-run` writes no posts or images, moves nothing and logs nothing — it only reports the slugs (the geocode cache may still be updated).
