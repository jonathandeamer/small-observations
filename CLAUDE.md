...
- **Related-posts strip on single pages.** `layouts/partials/related.html` renders a "Related" strip at the foot of each post, driven by Hugo's built-in `[related]` engine configured in `hugo.toml` (indices `artists` 90 > `tags` 70 > `cities` 40, `threshold = 80`, `includeNewer = true`, `toLower = false`). Two load-bearing details: the `{{ with $related }}` guard means the strip — heading and all — is **omitted entirely** when nothing clears the threshold (~30 of ~113 posts); don't "fix" this into an always-rendered empty `<aside>`. And the cards call `gallery-card.html` with `eager=false` on purpose — the post photo is the LCP image, so related thumbnails must never claim `loading=eager`/`fetchpriority`. `toLower` must stay `false` because index names map to mixed-case taxonomy params (`John Lennon`, `Liverpool FC`). See `docs/superpowers/specs/2026-06-25-related-posts-strip-design.md`. (On branch `feat/related-posts` until merged.)
